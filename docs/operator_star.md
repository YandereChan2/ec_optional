...
如果对象 _出错_ ，那么[行为未定义](https://zh.cppreference.com/w/cpp/language/ub)。
...
这些运算符重载不会检查 `*this` 是否 _含值_ ，可以通过 [`has_value`](has_value.md) 或 [`operator bool`](has_value.md) 自行检查，或者通过 [`error_code`](error_code.md) 查看错误码确定 _出错_ 状态。另外，也可以使用 [`value`](value.md) 来安全的访问所含值，它在实际 _出错_ 时会抛出异常。